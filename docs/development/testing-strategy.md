...
Attendance is covered across PostgreSQL-backed API integration tests, focused React/IndexedDB tests, and real Playwright Chromium tests. The Playwright global setup creates/resets only `koranco_e2e`; its Python seed refuses any other database. Browser scenarios exercise service-worker loading, offline refresh, pending submission, reconnection, response-loss replay, and cross-user isolation. Physical-device tests remain required because desktop automation cannot reproduce OS eviction, device custody, low-memory termination, or field conditions.
